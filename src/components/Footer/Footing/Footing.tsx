...
import type { ContentfulFootingSectionResponse, FootingCategoryProps, FootingProps } from "../../../util/types";
...
const Footing = () => {
  ...
  const dataArr: FootingCategoryProps[] = data?.data?.footer?.footersCollection?.items || [];
  return (
      ...
      {dataArr &&
        dataArr.map((category) => (
          <Links type="group">
            ...
            {category.linkCollection.items.map(
              (item: FootingProps) => (
                <Link url={item.url} title={item.title} styles={styles} />
              )
            )}
          </Links>
        ))}
